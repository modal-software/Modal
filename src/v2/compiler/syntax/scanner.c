...
static void lex_all(Tokenizer *t, _mvec_token *out)
{
    size_t per_chunk = CHUNK_SZ / sizeof(Token);
    size_t guess = ((size_t)t->pos / 2) + 16;
    *out = tokenvec.init(cx.pool, (guess / per_chunk) + 1);
    for (;;)
    {
        Token tok = next(t);
        tokenvec.push(out, tok);
        if (tok.kind == TOK_EOF)
        {
            return;
        }
