 ...
 *
 *   P5    Algorithmic audit + line-index [LANDED]:
 *         symptom: gates "fast, then progressively slower" — profiled to
 *         gate_check16 doing TWO tok_line_col() calls PER TOKEN, each a
 *         full src[0..off) newline rescan => O(tokens x bytes).
 *         - fix: LineIndex (sorted '\n' offsets, built ONCE per buffer via
 *           the existing vectorized span_non_newline span), lookups are
 *           upper-bound binary search O(log lines); scalar rescan kept for
 *           one-off cold error paths only
 *         - proof probe: -DLC_MICRO times v4-style rescan vs index lookup
 *           over a 64KB/~13k-token corpus per pass
 *
 *         LOOP CENSUS (57 'for' loops audited):
 *         hot path ....... 11 — of which restructured: tok_line_col rescan
 *           (replaced by index lookup; the only true quadratic). Deliberately
 *           NOT unrolled, with reasons:
 *           sym_hash FNV-1a: serially dependent chain (h=(h^b)*p) — an
 *             unroll cannot overlap anything; real lever would be a
 *             parallel-reducible hash or chunked fold (future work)
 *           digit fold v=v*10+d: same serial-dependence story; exact-semantics
 *             unroll is a no-op, radix-chunk accumulation changes overflow
 *             wrapping on >18 digits (doc D6; demo has <=2-digit numbers)
 *         oracle twins ... scalar ref_scan16/span_str_end loops stay dumb BY
 *           CONTRACT (their independence is what makes the gates worth
 *           trusting)
 *         init/cold ...... ~45 loops run once (table builds, self-tests,
 *           harness setup/reporting) — unrolling them optimizes nothing
 *
 *   P5b   D0 realistic-corpus bench [LANDED] — argv[9]=MB generates a
 *         grammar-valid corpus (decls inside test blocks: p_block pushes a
 *         fresh scope so a small name pool never redeclares; flat-precedence
 *         exprs: primary() has no parens), validates it end-to-end once,
 *         then benches it. resolved[] storage made persistent per stream
 *         (zero-fill once, reuse exact since same bytes -> same writes);
 *         pools scale with corpus size. MEASURED, HONEST (8MB, K=4, all
 *         workers ok; a first cut of these numbers was INVALID — a
 *         persistent-resolved[] variant tripped rsl_declare's zero-state
 *         guard from run #2 onward, so most "pipelines" were short-
 *         circuited errors that inflated throughput ~4-20x):
 *           lexed  avx2: 0.320 GB/s agg | 0.080 GB/s/stream | 61.0 ns/tok
 *           lexed  swar: 0.321 GB/s agg | 0.080 GB/s/stream | 60.9 ns/tok
 *           nolex      : 0.473 GB/s agg | 0.118 GB/s/stream | 41.3 ns/tok
 *         FINDINGS (corrected):
 *         - parse+resolve+fold is NOT multi-GB/s on real-size input: 41
 *           ns/token vs the demo's ~5. Cost drivers unique to big corpora:
 *           O(src_len) resolved[] zero-fill per run, one pool Obj alloc
 *           + Scope/mvec growth PER BLOCK accumulating until reset, cache
 *           misses on 8MB working set. These are the new #1/#2 levers,
 *           ahead of scanner fusion.
 *         - scanner adds ~20 ns/token in-pipeline (vs 8.5 isolated) —
 *           stream write+reload traffic still real; fusion still pays.
 *         - swar == avx2 at scale (the earlier +48% was the same failure
 *           artifact); level choice is not a lever here.
 *
 *   P5c   Scanner war round 1 [LANDED]:
 *         - forced-inline intern + tok16_set_len: 204 -> 200 ns/call only
 *           (calls were NOT the wall)
 *         - O2/O3/PGO sweep: scan_tokens16 pinned at 200 regardless;
 *           dumb ref_scan16 fell 259 -> 179 -> 168 — plain loops keep
 *           winning under better optimization, hand-SWAR is compiler-
 *           invariant => the tax had to be structural
 *         - monomorphic clones: scanner body instantiated once per SIMD
 *           level with direct spans (per-span g_simd dispatch deleted;
 *           dispatch once per SCAN). str-end escape loop moved inline
 *           into the body bound to each level's to-quote span.
 *           MEASURED: 194 ns/call isolated (-3%); bigcorp 58.3 vs 61.0
 *           ns/token (-4%); gates 6/6 across all three clones.
 *         - verdict: skeleton floor is real and now nearly bare. Next
 *           levers are architectural, not micro: fusion (kill the
 *           materialized stream's write+reload, worth ~2x in-pipeline)
 *           or chunked speculative parallel lexing of one stream.
 *
...
/* P5 scanner war: forced inline — this runs once PER TOKEN on the hot path;
 * the len>=255 branch is never taken on sane source, so after inlining the
 * common case is one compare+store. The escape path keeps its own cold
 * shape and shares BOTH scanners' semantics unchanged. */
__attribute__((always_inline)) static inline int tok16_set_len(Tok16 *t, size_t idx, size_t len)
{
...
/* P5 — line/col derivation, de-quadratized. v4's tok_line_col rescanned
 * src[0..off) on EVERY call; gate_check16 calls it TWICE PER TOKEN, so gate
 * cost grew as O(tokens x bytes) — fast on small corpora, progressively
 * slower as either grows (the "starts fast then crawls" profile).
 *
 * Fix: count newlines ONCE per buffer into a sorted offset index; lookup is
 * an upper-bound binary search -> O(log L) per token. The scalar rescan
 * stays for genuinely one-off cold error paths where building an index
 * would cost more than the single query.
 *
 * The leading '\n' sentinel makes the first body line number 1, matching
 * what scan-time bookkeeping produced before P4.5 (index includes it). */
/* forward decl: P5's index builder reuses the consolidated span engine
 * (defined below with the rest of the SIMD machinery). */
static inline size_t span_non_newline_d(const char *p, size_t limit);

typedef struct
{
    const uint32_t *nl; /* offsets of each '\n', ascending */
    size_t n;
} LineIndex;

/* Walks the buffer with the SAME vectorized non-newline span the scanner
 * uses (swar/sse2/avx2 by runtime dispatch) and records every '\n' offset.
 * nl[] capacity must be >= len (worst case: every byte a newline).
 * Once-per-buffer cost, so gates/error paths amortize it away. */
static void lc_build(const char *src, size_t len, uint32_t *nl, size_t *out_n)
{
    const char *p = src;
    ...
    while (p < end)
    {
        p += span_non_newline_d(p, (size_t)(end - p));
        if (p < end) /* *p == '\n' by span contract */
        {
            nl[k++] = (uint32_t)(p - src);
            p++;
...
/* line = number of newlines in [0,off); col = off - last_nl_off. Binary
 * search for the LAST newline offset <= off-1... careful: v4 counted
 * newlines at indices < off and col = off - (last such index + 1). With
 * nl[] ascending, that count = upper_bound(nl, off-1) position, and
 * ls = nl[count-1] + 1 when count > 0 else 0. */
static void lc_lookup(const LineIndex *li, uint32_t off, int *line, int *col)
{
    size_t lo = 0, hi = li->n;
    while (lo < hi)
    {
    ...
    *line = (int)lo;
    *col = (int)(off - (lo ? li->nl[lo - 1] + 1u : 0u)) + 1;
}

/* cold-path fallback: single query, no index worth building */
...
/* P5d: length is part of the memo key. Fused mode can legitimately call
 * intern twice at the SAME address with different lengths (a boundary-cut
 * fragment "v12", then the whole "v12345" after rescan) — pointer-only
 * comparison handed the fragment's id to the full word. */
static __thread unsigned char memo_l = 0;

/* P5 scanner war: forced inline — the memo-hit path (s == memo_s) is two
 * loads + a compare; a call/ret per identifier token cost more than the
 * work itself at ~4-6 ident tokens per demo scan. Miss path (hash+probe)
 * inlines too but stays rare after startup pre-interning. */
__attribute__((always_inline)) static inline int intern(const char *s, unsigned char len)
{
    if (s == memo_s && len == memo_l)
    {
        return memo_id;
    }
    int id = -1;
...
static LineIndex g_src_li;            /* P5: newline offsets of g_src_map, built once in main */

/* P5/D0: what the BENCH actually scans. Defaults to the demo source; argv[9]
 * swaps in a generated bigcorp corpus before workers spawn. Gates stay on
 * the demo corpus regardless — they are correctness anchors, not benches. */
static const char *g_bench_src = NULL;
static size_t g_bench_len = 0;
static size_t g_bench_ntok = 0; /* from validation scan, for ns/token reporting */
...
/* P5 SCANNER WAR — monomorphic per-level clones. Evidence chain: hollow-
 * EMIT ablation put the floor at ~198ns (per-token skeleton), entry-path
 * reshaping was parity, forced-inlining gained only 4ns, and O3+PGO left
 * scan_tokens16 pinned at 200ns while the DUMB oracle fell to 168ns — its
 * plain loops have no structural tax left to pay. The remaining one was
 * ours: every span call re-dispatched on g_simd (load + cmp + branch per
 * token, twice for idents/keywords paths). The body is now instantiated
 * ONCE PER LEVEL with direct level-specific spans; dispatch happens once
 * per scan in scan_tokens16. All clones flow through the same gates
 * (scan == ref at every forced level), so equivalence stays verified. */

/* P5d fused mode: a mid-source scan error inside a BOUNDARY-CAPPED chunk
 * may be an artifact of the artificial byte cap (a sliced ':=' or a string
 * straddling the line), not a real source error. fused_fill suppresses
 * these prints while probing adjusted boundaries and re-reports honestly
 * only if the error survives adjustment. Cold flag; error paths only. */
static int g_scan_quiet = 0;
#define SCAN_WARN(...)                                                                             \
    do                                                                                             \
    {                                                                                              \
        if (!g_scan_quiet)                                                                         \
...
#define EMIT(k, tp, ln)                                                                            \
    ...
    {                                                                                              \
        ...
        {                                                                                          \
            SCAN_WARN("scan error: too many long tokens\n");                                       \
            *err = 1;                                                                              \
            goto done;                                                                             \
        }                                                                                          \
        n++;                                                                                       \
...
#define SCAN_BODY(NAME, S_SPACE, S_IDENT, S_DIGIT, S_TOQUOTE)                                      \
    static size_t NAME(const char *src, size_t body_len, Tok16 *out, int *err)                     \
    {                                                                                              \
        esc_n = 0; /* fresh escape list per scan (TLS) */                                          \
        const char *p = src;                                                                       \
        const char *end = src + body_len; /* first sentinel byte */                                \
        size_t n = 0;                                                                              \
        int cur_line = 0; /* survives ONLY to print accurate scan errors */                        \
        ...
        {                                                                                          \
            ...
            case CLS_IDENT:                                                                        \
            {                                                                                      \
                size_t len = S_IDENT(p, (size_t)(end - p));                                        \
            ...
            case CLS_DIGIT:                                                                        \
            {                                                                                      \
                size_t nd = S_DIGIT(p, (size_t)(end - p));                                         \
                long long v = 0;                                                                   \
            ...
            case CLS_STR:                                                                          \
            {                                                                                      \
                /* closing-quote walk WITH backslash-escape handling, bound to  */                 \
                /* this level's to-quote span (was the shared span_str_end_d)   */                 \
                ...
                for (;;)                                                                           \
                {                                                                                  \
                    size_t q = base + S_TOQUOTE(s0 + base, lim - base);                            \
                    if (q >= lim)                                                                  \
                    {                                                                              \
                        qoff = lim;                                                                \
                        break;                                                                     \
                ...
                if (qoff == lim)                                                                   \
                {                                                                                  \
                    SCAN_WARN("scan error [%d:%d]: unterminated string\n", cur_line,               \
                              (int)(p - line_start) + 1 + (int)lim);                               \
                    *err = 1;                                                                      \
                    return n;                                                                      \
                }                                                                                  \
            ...
            case CLS_COLON:                                                                        \
                ...
                SCAN_WARN("scan error [%d:%d]: lone ':'\n", cur_line, (int)(p - line_start) + 1);  \
                *err = 1;                                                                          \
                return n;                                                                          \
            default:                                                                               \
                /* class value < TOK__COUNT => single-byte token of this kind */                   \
                if (cls < TOK__COUNT)                                                              \
                {                                                                                  \
                ...
                SCAN_WARN("scan error [%d:%d]: unexpected byte '%c'\n", cur_line,                  \
...
#ifndef MODAL_X86
SCAN_BODY(scan_tokens16_swar, span_space, span_ident, span_digit, span_to_quote)
#else
SCAN_BODY(scan_tokens16_swar, span_space, span_ident, span_digit, span_to_quote)
SCAN_BODY(scan_tokens16_sse2, span_space_sse2, span_ident_sse2, span_digit_sse2, span_to_quote_sse2)
SCAN_BODY(scan_tokens16_avx2, span_space_avx2, span_ident_avx2, span_digit_avx2, span_to_quote_avx2)
#endif
#undef EMIT
#undef SCAN_BODY
...
{
#ifndef MODAL_X86
    return scan_tokens16_swar(src, body_len, out, err);
#else
    switch (g_simd)
    {
    case SIMD_AVX2:
        return scan_tokens16_avx2(src, body_len, out, err);
    case SIMD_SSE2:
        return scan_tokens16_sse2(src, body_len, out, err);
    default:
        return scan_tokens16_swar(src, body_len, out, err);
    }
#endif
...
/* P5: takes a prebuilt LineIndex — the two per-token line/col derivations
 * that made v4's gates O(tokens x bytes) are now O(log lines) each. */
                        ...
                        const LineIndex *li)
{
    ...
    for (size_t i = 0; i < nr; i++)
    {
        ...
        int xl, xc, yl_, yc;
        lc_lookup(li, x->off, &xl, &xc);
        lc_lookup(li, y->off, &yl_, &yc);
...
/* ---------------------------------------------------------------------------
 * P5d — FUSED lexer (scan feeds parse directly, no materialized stream).
 *
 * Why: split mode pays the full Tok16 write (scan) AND reload (parse). At
 * 8MB in-pipeline that round-trip costs ~20 ns/token on top of ~8 ns of
 * isolated scan compute — the biggest remaining architectural lever.
 *
 * Shape: the parser horizon is exactly {previous, current} (audited: no
 * pointer arithmetic, `base` never read, tok_len never called by the
 * parser), so two ping-pong windows of FUSED_WIN tokens suffice. Refill
 * calls the EXISTING scan_tokens16 clones into the inactive window — zero
 * scanner duplication, one call amortized over FUSED_WIN tokens. A window
 * survives while `previous` points into it because the flip only rewrites
 * the OTHER window.
 *
 * Retention contract: nothing may hold a token pointer across an unbounded
 * number of advances. The one offender (p_const_decl's `name`) now copies
 * its 16B token. OpTok.tok survives flips unread in the fold build, so
 * fusion is compile-time refused under MODAL_KEEP_AST.
 *
 * Escapes: scan resets the TLS escape list per refill, so >254B tokens are
 * unsupported here (gates cover them in split mode only); detected once
 * and reported, never silent.
 *
 * Boundaries: the byte cap must never interact with multi-byte source
 * constructs. Word/number cuts are healed silently post-scan; a sliced
 * ':=' or a straddling string shows up as a mid-source scan error and is
 * repaired lazily by shrinking the boundary (see fused_fill). Soundness:
 * tokens-per-chunk <= bytes-per-chunk (every kind consumes >=1 byte), so
 * take <= FUSED_TAKE can never overflow the FUSED_TAKE+1 rows.
 * ------------------------------------------------------------------------- */
#define FUSED_TAKE 1023      /* bytes scanned per refill */
#define FUSED_WIN FUSED_TAKE /* token slots (rows = TAKE+1) */

#define TOK_HASH_SEED 1469598103934665603ull

/* Consumed-token digest for the fused-vs-split gate. Mixes every field so
 * "same kinds, wrong offsets" cannot pass. Both legs use this exact fn. */
static inline uint64_t tok_mix(uint64_t h, const Tok16 *t)
{
    uint64_t v = (uint64_t)t->kind | ((uint64_t)t->off << 8) | ((uint64_t)t->len << 40) |
                 ((uint64_t)(uint16_t)t->sym << 48);
    h ^= v + 0x9e3779b97f4a7c15ull + (h << 6) + (h >> 2);
    h ^= (uint64_t)t->num * 0xff51afd7ed558ccdull;
    return h ^ (h >> 29);
}
...
typedef struct
{
    const char *src;
    size_t body_len;
    const char *cur;             /* scan cursor; offset src+cur == bytes consumed */
    Tok16 win[2][FUSED_WIN + 1]; /* +1: scanner's own terminator slot */
    const Tok16 *wend;           /* one past last valid slot of the ACTIVE window */
    int active;
    int err;        /* latched scanner error from any refill */
    int esc_warned; /* >254B token / oversized string seen once -> warn once */
    uint64_t hash;  /* consumed-token digest when hashing */
    int hashing;
} FusedLex;
...
typedef struct
{
    ...
    const Tok16 *base;
    int had_error;
    FusedLex *fx;        /* NULL in split mode */
    uint64_t *hash_sink; /* NULL unless consumption is being hashed */
} Parser;

static void advance(Parser *p);
static void fused_advance(Parser *p);

static void advance(Parser *p)
{
    p->previous = p->current;
    if (__builtin_expect(p->hash_sink != NULL && p->previous->kind != TOK_EOF, 0))
    {
        *p->hash_sink = tok_mix(*p->hash_sink, p->previous);
    }
    if (p->current->kind != TOK_EOF)
    {
        if (__builtin_expect(p->fx != NULL, 0))
        {
            fused_advance(p);
            return;
        }
...
/* P5d: would byte `b` extend a word/number token? Used ONLY at chunk
 * boundaries to detect a token the bounded scan sliced in half. */
static int tok_cut_by(const Tok16 *lt, char b)
{
    unsigned char c = (unsigned char)b;
    switch (lt->kind)
    {
    case TOK_NUMBER:
        return g_cls[c] == CLS_DIGIT;
    case TOK_IDENTIFIER:
    case TOK_FUNCTION:
    case TOK_CONST:
    case TOK_TEST:
    case TOK_ASSERT:
    case TOK_WRITE:
        /* words continue on letters AND digits: v123 sliced at "v12|3"
         * must be caught (this exact bug shipped through small-corpus
         * gates because their idents never straddled a boundary) */
        return g_cls[c] == CLS_IDENT || g_cls[c] == CLS_DIGIT;
    default:
        return 0; /* ops/braces/strings/:= are self-delimiting */
    }
}

/* P5d: scan the next chunk into window `w`, latch errors, advance the
 * cursor. BOUNDED INPUT CONTRACT: the scanner has no token cap, so we cap
 * the BYTES -- every token consumes >=1 byte, hence tokens <= take <=
 * FUSED_TAKE always fits the FUSED_TAKE+1 rows at ANY density. take only
 * ever SHRINKS during boundary repair, preserving that bound.
 *
 * Boundary repair (lazy): a mid-source error in a capped chunk is treated
 * as a suspected artifact of the artificial cap -- a sliced ':=' (the
 * scanner sees a lone ':') or a string straddling the line (false
 * "unterminated"). Both are repaired by moving the boundary BACKWARD and
 * rescanning quietly; a word/number sliced mid-token needs no repair here
 * (the post-scan cut check backs up over it silently). Errors that survive
 * repair are re-reported loudly by one final scan. */
#define FUSED_RETRIES 8
static size_t fused_fill(FusedLex *fx, Tok16 *w)
{
    const char *cs = fx->cur;
    size_t rem = fx->body_len - (size_t)(cs - fx->src);
    size_t take = rem < FUSED_TAKE ? rem : FUSED_TAKE;
    int capped = take < rem;
    int err = 0;
    if (capped)
    {
    ...
    size_t n = scan_tokens16(cs, take, w, &err);
    for (int tries = 0; err && capped && tries < FUSED_RETRIES; tries++)
    {
        size_t nt;
        if (cs[take - 1] == ':')
        {
            nt = take - 1; /* sliced ':=': pair starts the next chunk */
        }
        else
        {
            /* suspect a straddling string: its opening quote is the last
             * '"' before the boundary (bounded backward walk) */
            size_t q = take;
            size_t floor_ = take > 256 ? take - 256 : 0;
            while (q > floor_ && cs[q - 1] != '"')
            {
                q--;
            }
            if (q == floor_)
            {
                break; /* no quote nearby: not a boundary artifact */
            }
            nt = q - 1; /* literal opens the NEXT chunk instead */
        }
        if (nt == 0)
        {
            break;
    ...
    if (capped)
    {
        ...
        if (err)
        {
            n = scan_tokens16(cs, take, w, &err); /* loud, honest report */
        }
    }

    /* REBASE: EMIT stores offsets relative to the chunk start it was
     * handed; parser/resolver need absolute source offsets. One add per
     * token, once per token overall. */
    uint32_t base_abs = (uint32_t)(cs - fx->src);
    if (err)
    {
        fx->err = 1;
        /* scanner error paths return WITHOUT a terminator: write one so
         * the parser drains cleanly instead of walking stale slots. off is
         * approximate (cold path; first error already printed above). */
        w[n].kind = TOK_EOF;
        w[n].len = 0;
        w[n].sym = -1;
        w[n].num = 0;
        w[n].off = (uint32_t)(n ? w[n - 1].off + base_abs : base_abs);
        n++;
        fx->cur = fx->src + fx->body_len;
    }
    else
    {
        if (!capped)
        {
            fx->cur = cs + take; /* real EOF sits in w[n-1]; freeze on it */
        }
        else
        {
            n--; /* mid-source: drop the pseudo-EOF */
            if (n > 0)
            {
                const Tok16 *lt = &w[n - 1];
                if (lt->off + lt->len == take && tok_cut_by(lt, cs[take]))
                {
                    /* rescan the cut token whole next chunk; progress is
                     * guaranteed: a token (<256B) always starts strictly
                     * before a 1023B boundary */
                    n--;
                    fx->cur = cs + lt->off;
                }
                else
                {
                    fx->cur = cs + take; /* trailing ws or clean boundary */
                }
            }
            else
            {
                fx->cur = cs + take; /* all-whitespace chunk */
            }
        }
    ...
    for (size_t i = 0; i < n && !fx->esc_warned; i++)
    {
        if (w[i].len == TOK16_LEN_ESC)
        {
            fx->esc_warned = 1;
            fprintf(stderr, "fused: >254B token unsupported (run split mode)\n");
        }
    }
    return n;
}

/* Called when current++ would leave the active window: flip. `previous`
 * keeps pointing into the OLD window, which stays intact for exactly one
 * more full drain cycle — that is the whole ping-pong invariant. */
static void fused_refill(Parser *p)
{
    FusedLex *fx = p->fx;
    int nxt = fx->active ^ 1;
    size_t n = fused_fill(fx, fx->win[nxt]);
    fx->active = nxt;
    p->current = fx->win[nxt];
    fx->wend = fx->win[nxt] + n;
}

static void fused_advance(Parser *p)
{
    FusedLex *fx = p->fx;
    const Tok16 *next = p->current + 1;
    if (__builtin_expect(next == fx->wend, 0))
    {
        fused_refill(p);
        return;
    }
...
/* First fill + parser bootstrap shared by every fused entry point.
 * NOTE: hashing/hash are CALLER-OWNED instrumentation — deliberately NOT
 * touched here (a gate that sets them before calling must keep them). */
static void fused_start(Parser *p, FusedLex *fx, const char *src, size_t len)
{
    fx->src = src;
    fx->body_len = len;
    fx->cur = src;
    ...
    fx->err = 0;
    fx->esc_warned = 0;
    size_t n = fused_fill(fx, fx->win[0]);
    fx->wend = fx->win[0] + n;
    *p = (Parser){.current = fx->win[0],
                  .previous = NULL,
                  .base = NULL,
                  .had_error = 0,
                  .fx = fx,
...
{
    advance(p);
    /* P5d: COPY, don't pin. `name` must survive every advance of the whole
     * initializer expression; fused windows flip underneath long exprs, and
     * even split mode reuses nothing here. 16B copy, once per decl. */
    Tok16 name = *p->current;
    if (!match(p, TOK_IDENTIFIER))
    {
...
/* P5d: parser+resolver core, shared by the split and fused entries. The
 * Parser (including its fx/hash_sink wiring) is fully configured by the
 * caller; everything below is mode-agnostic. */
static int pipeline_common(Parser *p, Cx *cx)
{
    t_cx = cx;
    Rsl r = {
        .cx = cx,
     ...
     * source, not the token count. One bulk NULL-fill, no pushes.
     * P5 REVERT NOTE: a "persistent resolved[] filled once" variant broke
     * rsl_declare, which READS the zero state as an internal freshness
     * guard ('already declared or resolved' spam from run #2 onward) and
     * silently invalidated every later pipeline. The zero-fill is part of
     * the contract; any future reuse scheme must be epoch- or dirty-list-
     * based and gated. */
...
static int run_pipeline_cx(Cx *cx, const Tok16 *stream, size_t ntokens)
{
    ...
    Parser p = {.current = stream,
                .previous = NULL,
                .base = stream,
                .had_error = 0,
                .fx = NULL,
                .hash_sink = NULL};
    return pipeline_common(&p, cx);
}

/* Gate-only variant: hash every token the parser actually CONSUMES.
 * Hashing the materialized stream instead would lie on error corpora,
 * where parsing stops before the stream's end. */
static int run_pipeline_cx_hashed(Cx *cx, const Tok16 *stream, size_t ntokens, uint64_t *sink)
{
    (void)ntokens;
    uint64_t h = TOK_HASH_SEED;
    Parser p = {.current = stream,
                .previous = NULL,
                .base = stream,
                .had_error = 0,
                .fx = NULL,
                .hash_sink = &h};
    int rc = pipeline_common(&p, cx);
    *sink = h;
    return rc;
}

#ifndef MODAL_KEEP_AST
/* P5d fused entry: no Tok16 stream exists; the parser pulls tokens through
 * FusedLex refills. Returns scanner error OR parser/resolver error, same
 * contract as run_pipeline_cx. */
static int run_pipeline_fused(Cx *cx, const char *src, size_t len, FusedLex *fx)
{
    Parser p;
    fused_start(&p, fx, src, len);
    if (fx->hashing)
    {
        p.hash_sink = &fx->hash;
    }
...
#else
#error "P5d fusion requires the fold-during-parse build: OpTok.tok survives window flips"
#endif
...
typedef struct
{
    ...
    FusedLex fx; /* P5d: fused lexer state (windows + cursor), reused */
} Stream;
...
/* P5d: where tokens come from each pipeline run.
 *   NOLEX  replay the prepared stream (parity mode)
 *   SPLIT  scan_tokens16 -> materialized Tok16 array -> parse (was "lex")
 *   FUSED  parser pulls tokens through FusedLex; no materialized stream */
typedef enum
{
    LEXMODE_NOLEX = 0,
    LEXMODE_SPLIT = 1,
...
typedef struct
{
    ...
    /* P4.1: when lexmode is SPLIT, workers scan g_src into a fresh token
     * array each run instead of replaying `tokens`. NOLEX restores phase-3
     * semantics exactly (parity mode for A/B); FUSED (P5d) skips the
     * materialized stream entirely. */
    LexMode lexmode;
} GlobalCfg;
    ...
    for (uint64_t r = lo; r < hi; r++)
    {
        for (int s = 0; s < g->nstreams; s++)
        {
            ...
            if (g->lexmode == LEXMODE_SPLIT)
            {
                ...
                int serr = 0;
                size_t nt = scan_tokens16(g_bench_src, g_bench_len, streams[s].toks, &serr);
                *had_err |= serr;
                *had_err |= run_pipeline_cx(&streams[s].cx, streams[s].toks, nt);
            }
            else if (g->lexmode == LEXMODE_FUSED)
            {
                /* P5d path: no materialized stream. FusedLex state lives in
                 * the Stream and is fully re-initialized by fused_start. */
                *had_err |=
                    run_pipeline_fused(&streams[s].cx, g_bench_src, g_bench_len, &streams[s].fx);
            }
            else
            {
...
/* P5/D0 — realistic-corpus generator. Deterministic (xorshift32), grammar-
 * valid Modal: test blocks with const decls, asserts, writes. Identifiers
 * cycle through a bounded pool (~256 names) like real code reuses names —
 * also keeps intern within SYM_TAB_MAX. No '/' in exprs: fold-during-parse
 * would divide by a possibly-zero folded subexpression. Tail replicates the
 * g_src sentinel contract ("' \n + NULs, >=16 readable bytes past body). */
static char *bigcorp_build(size_t target, size_t *out_len)
{
    size_t page = (size_t)sysconf(_SC_PAGESIZE);
    size_t map_sz = (target + 64 + page - 1) & ~(page - 1);
    char *buf = mmap(NULL, map_sz, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (buf == MAP_FAILED)
    {
        return NULL;
    }
    uint32_t rng = 0x12345678u;
#define BC_NEXT() (rng ^= rng << 13, rng ^= rng >> 17, rng ^= rng << 5)
    size_t off = 0;
    while (off + 512 < target)
    {
        uint32_t a = BC_NEXT() % 1000u, b = BC_NEXT() % 1000u, c = BC_NEXT() % 999u;
        unsigned n1 = BC_NEXT() % 256u, n2 = BC_NEXT() % 256u;
        /* Shape proven against the real grammar: decls live INSIDE test
         * blocks (p_block dispatches g_decl_map and pushes a fresh scope,
         * so cycling a small name pool never redeclares), expressions are
         * FLAT precedence — primary() accepts only numbers/identifiers.
         * Folded values stay >= 0; intern stays well under SYM_TAB_MAX. */
...
#undef BC_NEXT
    ...
    buf[off++] = '\'';
    buf[off++] = '\n';
    memset(buf + off, 0, 16); /* sentinel pad: >=8B safe loads past body */
    *out_len = body_end;      /* body length EXCLUDES the sentinel tail */
    return buf;
}

/* Build the bench corpus and validate it end-to-end once (scan == parse ==
 * resolve == fold clean) so workers never measure error paths by accident.
 * Returns token count. keep_toks != NULL hands the scanned stream back for
 * nolex replay (caller frees nothing; process-exit lifetime). */
static size_t bigcorp_setup(long mb, LFPool *scratch, Tok16 **keep_toks)
{
    size_t target = (size_t)mb << 20;
    size_t blen = 0;
    char *src = bigcorp_build(target, &blen);
    if (!src)
    {
        fprintf(stderr, "fatal: bigcorp mmap failed\n");
        exit(1);
    }
    Tok16 *toks = aligned_alloc(64, ((blen / 4 + 2) * sizeof(Tok16) + 63) & ~(size_t)63);
    int err = 0;
    size_t nt = scan_tokens16(src, blen, toks, &err);
    if (err || nt == 0)
    {
        fprintf(stderr, "fatal: bigcorp scan failed\n");
        exit(1);
    }
    Cx cx = {.pool = scratch, .src = src, .src_len = blen, .verbose = 0};
    t_cx = &cx;
    if (run_pipeline_cx(&cx, toks, nt))
    {
        fprintf(stderr, "fatal: bigcorp pipeline rejected generated source\n");
        exit(1);
    }
    if (keep_toks)
    {
        *keep_toks = toks; /* validated replay stream */
    }
    else
    {
        free(toks);
...
static void *bench_worker(void *arg)
{
     ...
     * upper bound can never grow, so no growth checks and no allocator
     * traffic inside the timed region.
     * P5/D0: bigcorp caps slots at len/4 (generated text measures ~4.8
     * bytes/token, so /4 keeps a >20% margin without 16B-per-byte buffers). */
    size_t body = g_bench_len;
    size_t nslots = g_bench_src == g_src_map ? body + 2 : body / 4;
    size_t toks_sz = (nslots * sizeof(Tok16) + 63) & ~(size_t)63;
    /* P5/D0: one pipeline run over the bigcorp corpus allocates a Scope
     * (+obj vectors) PER BLOCK and only pool.reset() reclaims them, so the
     * slab must scale with block count (~blen/95 blocks x ~150B) instead of
     * staying at the demo's 4KB. */
    size_t pcap = (g_bench_src == g_src_map) ? POOL_CAPACITY : g_bench_len * 2 + (1u << 20);
    for (int s = 0; s < g->nstreams; s++)
    {
        if (!pool.allocator(&streams[s].pool, pcap))
        {
        ...
        memset(&streams[s].fx, 0, sizeof streams[s].fx); /* no instrumentation in bench */
        streams[s].cx = (Cx){
            .pool = streams[s].pool,
            .src = g_bench_src,
            .src_len = g_bench_len,
...
#define MICRO_N 50000000ull
    ...
    /* P5 proof probe: v4 gates derived line/col by FULL RESCAN per token
     * (2x/token) — O(tokens x bytes), the "fast then progressively slower"
     * profile. Same work against the P5 newline index: O(log lines)/token.
     * Synthetic 64KB corpus, ~13k tokens, one pass of gate-style checks. */
    {
        const size_t BL = 1u << 16;
        char *big = malloc(BL + 64);
        memcpy(big, "\n", 1);
        size_t blen = 1;
        while (blen < BL)
        {
            memcpy(big + blen, "const ab := 123\nwrite \"hi\"\n", 27);
            blen += 27;
        }
        memset(big + blen, 0, 32);
        static Tok16 btoks[1u << 16];
        int berr = 0;
        size_t ntok = scan_tokens16(big, blen, btoks, &berr);

        uint32_t *nl = malloc((blen + 2) * sizeof *nl);
        LineIndex bli;
        lc_build(big, blen, nl, &bli.n);
        bli.nl = nl;

        const int PASSES = 20; /* v4 cost grows with tokens*bytes; show it */
        uint64_t t0 = now_ns();
        long long chk = 0;
        for (int r = 0; r < PASSES; r++)
        {
            ...
            {
                int a, b, c, d;
                tok_line_col(big, btoks[i].off, &a, &b); /* v4 pattern */
                tok_line_col(big, btoks[i].off, &c, &d);
                chk += a + b + c + d;
            }
        }
        uint64_t scan_style = now_ns() - t0;
        ...
        for (int r = 0; r < PASSES; r++)
        {
            ...
            {
                int a, b, c, d;
                lc_lookup(&bli, btoks[i].off, &a, &b); /* P5 path */
                lc_lookup(&bli, btoks[i].off, &c, &d);
                chk += a + b + c + d;
            }
        }
        uint64_t idx_style = now_ns() - t0;
        printf("linecol v4 rescan: %llu us/pass (%zu tok) | P5 index: %llu us/pass | sink=%lld\n",
               (unsigned long long)(scan_style / PASSES / 1000), ntok,
               (unsigned long long)(idx_style / PASSES / 1000), chk % 1000000);
        free(big);
        free(nl);
    }
    free(buf);
...
#elif defined(FUSED_MICRO)
/* P5d interleaved duel: split (scan -> stream -> parse) vs fused (scan
 * feeds parse directly), ALTERNATED rep-by-rep in ONE process so throttle
 * drift hits both legs equally (binding methodology). Proves consumed-hash
 * equality once, then times un-instrumented legs.
 * Build: -DFUSED_MICRO [-DDUEL_MB=1] [-DDUEL_REPS=40] */
int main(void)
{
    class_tables_init();
    build_dispatch_maps();
...
#endif
    size_t blen = 0;
    char *src = bigcorp_build((size_t)mb << 20, &blen);
    if (!src)
    {
        fprintf(stderr, "duel: corpus build failed\n");
        return 1;
    }

    LFPool *pl;
    if (!pool.allocator(&pl, blen * 2 + (1u << 20)))
    {
        return 1;
    }
    Cx cx = {.pool = pl, .src = src, .src_len = blen, .verbose = 0};
    t_cx = &cx;

    Tok16 *toks = aligned_alloc(64, ((blen / 4 + 2) * sizeof(Tok16) + 63) & ~(size_t)63);
    int serr = 0;
    size_t nt = scan_tokens16(src, blen, toks, &serr);
    if (serr || nt == 0)
    {
        fprintf(stderr, "duel: scan failed\n");
        return 1;
    }

    /* one-time equivalence proof with instrumentation ON */
    uint64_t hs = TOK_HASH_SEED;
    FusedLex fx;
    memset(&fx, 0, sizeof fx);
    fx.hashing = 1;
    fx.hash = TOK_HASH_SEED;
    pool.reset(pl);
    if (run_pipeline_cx_hashed(&cx, toks, nt, &hs))
    {
        fprintf(stderr, "duel: split leg errored\n");
        return 1;
    }
    pool.reset(pl);
    if (run_pipeline_fused(&cx, src, blen, &fx))
    {
        fprintf(stderr, "duel: fused leg errored\n");
        return 1;
    }
    if (hs != fx.hash)
    {
        fprintf(stderr, "duel: HASH MISMATCH hsplit=%016llx hfused=%016llx\n",
                (unsigned long long)hs, (unsigned long long)fx.hash);
        return 1;
    }
    printf("duel: equivalence ok (%zu tokens, h=%016llx)\n", nt, (unsigned long long)hs);

#ifdef DUEL_REPS
#if DUEL_REPS < 6
#error "duel needs >=6 reps per leg (methodology)"
#endif
    enum
    {
        REPS = DUEL_REPS
...
#endif
    static uint64_t tsp[REPS], tfp[REPS];
    uint64_t sink = 0;
    fx.hashing = 0; /* timed legs carry zero instrumentation */
    /* split leg must include its OWN scan (scan->stream->parse); timing
     * run_pipeline_cx alone would race fused against a nolex phantom */
    Tok16 *stoks = aligned_alloc(64, ((blen / 4 + 2) * sizeof(Tok16) + 63) & ~(size_t)63);
    if (!stoks)
    {
        return 1;
    }
    for (int r = 0; r < REPS; r++)
    {
        ...
        pool.reset(pl);
        uint64_t t0 = now_ns();
        size_t n2 = scan_tokens16(src, blen, stoks, &serr2);
        sink += (uint64_t)run_pipeline_cx(&cx, stoks, n2);
        tsp[r] = now_ns() - t0;

        pool.reset(pl);
        t0 = now_ns();
        sink += (uint64_t)run_pipeline_fused(&cx, src, blen, &fx);
        tfp[r] = now_ns() - t0;
    }

    for (int i = 1; i < REPS; i++) /* insertion sort both (tiny n) */
    {
        uint64_t a = tsp[i], b = tfp[i];
        int j = i - 1;
        for (; j >= 0 && tsp[j] > a; j--)
        {
            tsp[j + 1] = tsp[j];
    ...
    uint64_t ms = tsp[REPS / 2], mf = tfp[REPS / 2];
    printf("duel split : %llu ns/tok | %6.3f GB/s/stream | min %llu\n",
           (unsigned long long)(ms / nt), (double)blen / (double)ms, (unsigned long long)tsp[0]);
    printf("duel fused : %llu ns/tok | %6.3f GB/s/stream | min %llu\n",
           (unsigned long long)(mf / nt), (double)blen / (double)mf, (unsigned long long)tfp[0]);
    printf("duel ratio fused/split: %.3f (sink=%llu)\n", (double)mf / (double)ms,
...
int main(int argc, char **argv)
{
    ...
    /* Optional argv[7]: token-source mode. "nolex" replays the prepared
     * token array like phase 3 (parity mode); "lex" (default) scans raw
     * bytes into a materialized stream every run; "fused" (P5d) feeds the
     * parser straight from the scanner with no stream in between. */
    LexMode lexmode = LEXMODE_SPLIT;
    if (argc > 7)
    {
        if (strcmp(argv[7], "nolex") == 0)
        {
            lexmode = LEXMODE_NOLEX;
        }
        else if (strcmp(argv[7], "lex") == 0)
        {
            lexmode = LEXMODE_SPLIT;
        }
        else if (strcmp(argv[7], "fused") == 0)
        {
            lexmode = LEXMODE_FUSED;
        }
        else
        {
            fprintf(stderr, "fatal: unknown lex mode '%s' (nolex|lex|fused)\n", argv[7]);
            return 1;
        }
    }
    ...
    /* P5/D0: optional argv[9] = bigcorp size in MB. Replaces the 77B demo
     * source with a generated, grammar-valid corpus of that size so
     * GB/s-per-stream becomes a MEASURED number instead of an extrapolation
     * from fixed-cost-dominated toy runs. */
    long bigcorp_mb = 0;
    if (argc > 9)
    {
        bigcorp_mb = strtol(argv[9], NULL, 10);
    }
    ...
    /* P5: newline index for the mapped source — built ONCE, shared by all
     * gates and cold error lookups that want the fast path. */
    {
        static uint32_t src_nl[G_SRC_BODY_LEN + 2];
        g_src_li.nl = src_nl;
        lc_build(g_src_map, G_SRC_BODY_LEN, src_nl, &g_src_li.n);
    }

    /* P5/D0: bench-source defaults (demo corpus) — bigcorp_setup below
     * overrides both before workers spawn. */
    g_bench_src = g_src_map;
    g_bench_len = G_SRC_BODY_LEN;
     ...
     * num/sym + derived line/col vs recorded positions) before workers spawn. */
    {
        ...
        for (int lvl = SIMD_SWAR; lvl <= (int)simd_detect(); lvl++)
        {
            ...
            int ok = !gerr && gate_check16(g_src_map, prep_happy, n_happy, plens, pline, pcol,
                                           gate_buf, ng, &g_src_li);
     ...
     * 1-byte length escape identically in BOTH scanners. */
    {
        ...
        /* P5: per-buffer newline index (same de-quadratization as the main
         * gate — this corpus exists to stress long tokens, so per-token
         * rescans would dominate here first). */
        static uint32_t big_nl[768];
        static LineIndex big_li;
        big_li.nl = big_nl;
        lc_build(big, blen, big_nl, &big_li.n);
        if (eerr || !gate_check16(big, rbuf, nr, elens, rl, rc, gbuf, ngot, &big_li))
        {
    ...
    /* P5d fused-vs-split equivalence: the fused parser must consume the
     * EXACT token sequence the split parser consumes — on the happy path,
     * an error path (where parsing stops mid-stream), and a flip-stress
     * corpus. Proof = hash of every token each leg ACTUALLY advanced over.
     * Hashing the materialized stream instead would silently accept
     * truncated parses, so both legs instrument consumption itself. */
    {
        LFPool *gp;
        if (!pool.allocator(&gp, POOL_CAPACITY))
        {
            fprintf(stderr, "fatal: fused-gate pool init failed\n");
            return 1;
        }
        /* longexpr: 300-term flat initializer forces >=4 window flips while
         * p_const_decl's copied `name` must survive all of them. This is the
         * regression test for the retention-bug class fusion introduces. */
        static char long_src[2048];
        long_src[0] = '\n';
        size_t ll_ = 1;
        ll_ += (size_t)sprintf(long_src + ll_, "const big := 1");
        for (int i = 0; i < 300; i++)
        {
            ll_ += (size_t)sprintf(long_src + ll_, "+1");
        }
        ll_ += (size_t)sprintf(long_src + ll_, "\nwrite \"x\"\n");
        memset(long_src + ll_, 0, 32);
        static Tok16 long_toks[1024];
        int lerr = 0;
        size_t n_long = scan_tokens16(long_src, ll_, long_toks, &lerr);
        if (lerr)
        {
            fprintf(stderr, "fatal: fused-gate longexpr scan failed\n");
            return 1;
        }

        /* straddler: an IDENTIFIER deliberately spanning bytes [1021,1027)
         * across the FUSED_TAKE=1023 boundary ("v12|345"). Regression lock
         * for word-cut detection: words continue on digits too, and the
         * sliced fragment used to slip through as its own interned sym. */
        static char strad_src[2048];
        size_t sp_ = (size_t)sprintf(strad_src, "\n");
        while (sp_ < 1000)
        {
            sp_ += (size_t)sprintf(strad_src + sp_, "const z%zu := 5\n", sp_);
        }
        while (sp_ < 1015)
        {
            strad_src[sp_++] = ' ';
        }
        sp_ += (size_t)sprintf(strad_src + sp_, "const v12345 := 7\n");
        /* second hazard: ':' placed at offset 1022 so ':=' straddles the
         * boundary — the scanner must never see a chunk ending on ':' */
        while (sp_ < 1014)
        {
            strad_src[sp_++] = ' ';
        }
        sp_ += (size_t)sprintf(strad_src + sp_, "const u := 9\n");
        sp_ += (size_t)sprintf(strad_src + sp_, "write \"ok\"\n");
        memset(strad_src + sp_, 0, 32);
        static Tok16 strad_toks[512];
        int serr2 = 0;
        size_t n_strad = scan_tokens16(strad_src, sp_, strad_toks, &serr2);
        if (serr2)
        {
            fprintf(stderr, "fatal: fused-gate straddler scan failed\n");
            return 1;
        }
        /* prove the fixture actually straddles before trusting the gate */
        int strad_ok = 0;
        for (size_t i = 0; i < n_strad; i++)
        {
            if (strad_toks[i].kind == TOK_IDENTIFIER && strad_toks[i].off <= 1023u &&
                strad_toks[i].off + strad_toks[i].len > 1023u)
            {
                strad_ok = 1;
            }
        }
        if (!strad_ok)
        {
            fprintf(stderr, "fatal: fused-gate straddler fixture misplaced\n");
            return 1;
        }
        ...
        const struct
        {
            ...
            {"happy", g_src_map, G_SRC_BODY_LEN, prep_happy, n_happy, 0},
            {"redecl", redecl_src, sizeof(redecl_src) - 1, redecl, n_redecl, 1},
            {"longexpr", long_src, ll_, long_toks, n_long, 0},
            {"straddler", strad_src, sp_, strad_toks, n_strad, 0},
        };
        for (size_t c = 0; c < sizeof(cases) / sizeof(cases[0]); c++)
        {
            Cx cx = {.pool = gp, .src = cases[c].src, .src_len = cases[c].len, .verbose = 0};
            t_cx = &cx;
            uint64_t hs = TOK_HASH_SEED;
            int e_split = run_pipeline_cx_hashed(&cx, cases[c].stream, cases[c].ntok, &hs);
            ...
            fx.hashing = 1;
            fx.hash = TOK_HASH_SEED;
            int e_fused = run_pipeline_fused(&cx, cases[c].src, cases[c].len, &fx);

            int ok = e_split == cases[c].want_err && e_fused == cases[c].want_err && hs == fx.hash;
            printf("gate: fused == split [%s]%s (h=%016llx)\n", cases[c].name,
                   ok ? "" : "  <-- FAIL", (unsigned long long)fx.hash);
            if (!ok)
            {
                fprintf(stderr,
                        "fatal: fused gate FAILED [%s] split=%d fused=%d hsplit=%016llx "
                        "hfused=%016llx\n",
                        cases[c].name, e_split, e_fused, (unsigned long long)hs,
                        (unsigned long long)fx.hash);
                return 1;
            }
        }
        pool.drop(gp);
    ...
    /* P5/D0: build + validate the bench corpus before any harness config. */
    const Tok16 *replay = prep_happy;
    size_t replay_n = n_happy;
    if (bigcorp_mb > 0)
    {
        LFPool *scratch = NULL;
        if (!pool.allocator(&scratch, (size_t)bigcorp_mb * 2 << 20))
        {
            fprintf(stderr, "fatal: bigcorp scratch pool failed\n");
            return 1;
        }
        Tok16 *keep = NULL;
        size_t nt_big = bigcorp_setup(bigcorp_mb, scratch, lexmode == LEXMODE_NOLEX ? &keep : NULL);
        if (keep)
        {
            replay = keep;
            replay_n = keep ? nt_big : replay_n;
        }
        printf("bigcorp: %zu B | %zu tokens | %.2f B/token\n\n", g_bench_len, g_bench_ntok,
               (double)g_bench_len / (double)g_bench_ntok);
    }
    ...
    /* P5/D0: GB/s accounting. ns/op is WALL time per pipeline across all
     * workers, so aggregate bandwidth = corpus_bytes * runs / wall_ns and
     * per-stream = aggregate / K. This is the number the 45M-lines/s doc
     * target and any GB/s ambition must be judged against — measured, not
     * extrapolated from the fixed-cost-dominated demo corpus. */
    {
        double agg_gbps = (double)g_bench_len * (double)total_runs / (double)dt; /* B/ns = GB/s */
        double per_stream = agg_gbps / (double)K;
        size_t ntok = g_bench_ntok ? g_bench_ntok : 24;
        printf("throughput: %.3f GB/s aggregate | %.3f GB/s per stream | %.2f ns/token\n", agg_gbps,
               per_stream, (double)dt / (double)total_runs * (double)K / (double)ntok);
    }
