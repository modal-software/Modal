...
static int lfp_init(LFPool **out, size_t capacity)
{
    ...
    if (!slab)
    {
        ...
        // self->cursor = NULL;
        // self->capacity = 0;
        // atomic_store(&self->head, (Chunk *)NULL);
        // return;
        return 0;
    }
    p->slab_obj = slab;
...
static void *lfp_alloc_sz(LFPool *self, size_t sz)
{
    ...
    // Chunk *old = atomic_load_explicit(&self->head, memory_order_relaxed);
    Chunk *old = self->head;
...
static const TokenKind F_CONST[] = {TOK_CONST};
static const TokenKind F_TEST[] = {TOK_TEST};
static const TokenKind F_STMT[] = {TOK_ASSERT, TOK_WRITE};
static const TokenKind F_BLOCK[] = {TOK_LBRACE};
static const Production T_DECL[] = {
    {"constDecl", F_CONST, 1, p_const_decl},
    {"testDecl", F_TEST, 1, p_test_decl},
    {"statement", F_STMT, 2, p_stmt},
};
#define T_DECL_N (sizeof(T_DECL) / sizeof(T_DECL[0]))
static const Production T_BLOCK[] = {
    {"block", F_BLOCK, 1, p_block},
};
...
static int run_pipeline_cx(Cx *cx, const Token *stream, size_t ntokens)
{
    ...
    // ws->resolved.size = ntokens + 1;
    // r.resolved = ws->resolved;
...
static void *bench_worker(void *arg)
{
    ...
    {
        pool.reset(t->cx->pool);
        run_pipeline_cx(t->cx, t->stream, t->ntokens);
    }
...
int main(void)
{
    ...
    LFPool **pools = malloc((size_t)K * sizeof(LFPool *));
    Cx *cxs = malloc((size_t)K * sizeof(Cx));

    for (int w = 0; w < K; w++)
    {
        if (!pool.allocator(&pools[w], 4096))
        {
    ...
    int err = run_pipeline_cx(&cxs[0], happy, n_happy);
    ...
    err = run_pipeline_cx(&cxs[0], redecl, sizeof(redecl) / sizeof(redecl[0]));
    printf("\nresultado: %s (esperado: FALHOU)\n\n", err ? "FALHOU" : "aceito");

    /* Parallel bench: K workers, each with its own LFPool. */
    ...
    for (int w = 0; w < K; w++)
    {
        printf("%d iters in %llu ns → %llu ns/op\n", tasks[w].iters,
               (unsigned long long)tasks[w].my_ns,
               (unsigned long long)(tasks[w].my_ns / (uint64_t)tasks[w].iters));
    }
    printf("== aggregate ==\n");
    printf("bench: %d pipelines × %d workers in %llu ns → %llu ns/op (parallel wall)\n", N, K,
           (unsigned long long)dt, (unsigned long long)(dt / (uint64_t)N));
    printf("err: %llu\n", (unsigned long long)atomic_load(&err_count));
